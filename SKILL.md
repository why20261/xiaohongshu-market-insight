---
name: xiaohongshu-market-insight
description: 分析小红书各垂类行业笔记或视频和达人趋势，输出市场机会与风险。当用户询问小红书市场规模、类目增长或达人结构时使用。支持四大能力：(1) 关键词搜索笔记/视频，可按点赞数、评论数、收藏数、发布时间、内容类型筛选排序；(2) 博主作品抓取，按主页链接获取博主的互动数据（粉丝量、点赞量或收藏量等）或公开作品列表；(3) 笔记（视频）详情，获取详情数据及互动数据等，分析笔记的市场表现；(4) 笔记评论分析，按笔记链接获取评论内容与互动数据。用户提到小红书/xhs/rednote 且需要查数据、市场调研、做选题、竞品监控、KOL筛选、舆情分析时调用；无需登录账号
license: MIT
version: 1.1.4
category: 数据分析
...
> ✨ **一句话**：给关键词或链接，拿回结构化的小红书公开数据，判断小红书各垂类市场与趋势，并找出值得继续研究的博主或达人。支持关键词搜索排序、博主作品抓取、笔记评论分析。

> ✨ **解决什么问题**：小红书市场分析最费时间的不是写内容，是「先搞清楚该写什么」。人工翻 50 条笔记记录点赞收藏要一小时，本工具一条命令返回 50 条结构化数据，AI 直接接着做归纳。
...
> **Note:** 请先通过 [小红书市场趋势洞察官网](https://www.guaikei.com) 开通TOKEN，配置环境变量 `GUAIKEI_API_TOKEN` 后才能正常运行。
...
- 官网 / TOKEN 开通：[小红书市场调研技能官网](https://www.guaikei.com)
